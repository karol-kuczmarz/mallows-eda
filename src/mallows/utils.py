...
def estimate_theta(samples, sigma_0):
    def f(theta, V, n):
        val = 0.
        val += (n-1.) / (np.exp(theta) - 1)
        val -= np.sum((n - np.arange(1,n) + 1) / (np.exp(theta*(n - np.arange(1,n) + 1)) - 1))
        val -= V
        return val
    ...
    def f_prime(theta, n):
        ...
        val += np.sum((n - np.arange(1,n) + 1)**2. * (np.exp(theta * (n - np.arange(1,n) + 1))) / (np.exp(theta * (n - np.arange(1,n) + 1)) - 1)**2.)
        return val
    ...
    for i in range(sigma_0.shape[-1] - 1):
        V_hat[i] = np.sum(samples_sigma_0_inv[:, i+1:].T < samples_sigma_0_inv[:, i]) / samples.shape[-2]
    V = np.sum(V_hat)
    theta = newton_raphson(lambda x: f(x, V, sigma_0.shape[-1]), lambda x: f_prime(x, sigma_0.shape[-1]), 0.01)
    return theta
